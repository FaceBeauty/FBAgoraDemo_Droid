...
public class ProcessRawData extends BaseFragment implements View.OnClickListener {
    ...
    private final IVideoFrameObserver iVideoFrameObserver = new IVideoFrameObserver() {
        ...
        public boolean onCaptureVideoFrame(int sourceType, VideoFrame videoFrame) {
            ...
            // 初始化 TextureBufferHelper，只需初始化一次
            if (mTextureBufferHelper == null) {
                mTextureBufferHelper = TextureBufferHelper.create("VideoFrameProcessor", EglBaseProvider.instance().getRootEglBase().getEglBaseContext());
                if (mTextureBufferHelper != null) {
            ...
            // 使用 TextureBufferHelper 进行操作，确保 OpenGL 操作在同一线程中进行
            if (mTextureBufferHelper != null) {
                mTextureBufferHelper.invoke(() -> {

                    // 切换摄像头时释放 Buffer 渲染器
                    if (isCameraSwitched) {
                        FBEffect.shareInstance().releaseBufferRenderer();
                        isRenderInit = false;
                    ...
                    if (backBuffer == null || backBuffer.capacity() < nv21MinSize) {
                        backBuffer = ByteBuffer.allocateDirect(nv21MinSize);
                        backData = new byte[nv21MinSize];
                    }
                    ...
                    // 处理 NV21 数据
                    FBEffect.shareInstance().processBuffer(nv21);
                    ...
                    // 交换前后缓冲区
                    ByteBuffer tempBuffer = frontBuffer;
                    frontBuffer = backBuffer;
                    backBuffer = tempBuffer;

                    byte[] tempData = frontData;
                    frontData = backData;
                    backData = tempData;
